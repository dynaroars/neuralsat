...
class LinearSolver:
    ...
    def simplex_feasible(self, assignment):
        ...
        # print(self.parsed_input.vars_dict)
        # print(self.parsed_input.dict_var_to_eqn)
        # print(self.parsed_input.input_vars_dict)

        n_vars = len(self.parsed_input.input_vars_dict)
        A = []
        b = []
        c = [0 for _ in range(n_vars)]
        bounds = [(None, None) for _ in range(n_vars)]
        for var in assignment:
            if 'q' in var:
                if assignment[var]:
                    int_var = self.parsed_input.vars_dict[var]
                else:
                    int_var = self.parsed_input.vars_dict['~'+var]
                equation = self.parsed_input.dict_var_to_eqn[int_var]
                # construct A matrix
                A_row = np.zeros(n_vars)
                for k, v in equation.left_hand_side.items():
                    A_row[self.parsed_input.input_vars_dict[k]] = v
                A.append(A_row)
                # construct b matrix
                b.append(equation.right_hand_side)

        if not len(A) or linprog(c, A_ub=A, b_ub=b, bounds=bounds):
            return True

        # self.my_simplex = Simplex(self.parsed_input, rows)
        # if self.my_simplex.solve():
        #     return self.my_simplex
        # print('cac', assignment)
        # exit()
        return False
    ...
    def solve(self):
        ...
        real_sol = None
        if ret != False and self.my_simplex != None:
            real_sol = self.my_simplex.get_assignment()
        return ret, real_sol
