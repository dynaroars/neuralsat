...
class ConfigHandler:
    ...
    def add_common_options(self):
        ...
        self.add_argument("--seed", type=int, default=0, help='Random seed.', hierarchy=h + ["seed"])
        self.add_argument("--conv_mode", default="patches", choices=["patches", "matrix"], help='Convolution mode during bound propagation: "patches" mode (default) is very efficient, but may not support all architecture; "matrix" mode is slow but supports all architectures.', hierarchy=h + ["conv_mode"])
        ...
        self.add_argument('--init_iteration', type=int, default=10, help='Number of iterations for alpha-CROWN incomplete verifier.', hierarchy=h + ["iteration"])
        ...
        h = ["bab", "branching"]
        self.add_argument("--branching_method", default="babsr", choices=["babsr", "fsb", "kfsb", "sb"], help='Branching heuristic. babsr is fast but less accurate; fsb is slow but most accurate; kfsb is usualy a balance.', hierarchy=h + ["method"])
    ...
    def dump_config(self, args_dict, level=[], out_to_doc=False, show_help=False, indent=4):
        ...
        for key, val in args_dict.items():
            if isinstance(val, dict):
                ...
                if len(ret) > 0:
                    ...
                    ret_string += ' ' * (len(level) * indent) + f'{key}:\n' + ret
            else:
                if show_help:
                ...
                ret_string += ' ' * (len(level) * indent) + f'{yaml_line}{h}\n'
        if len(level) > 0:
            return ret_string
        else:
