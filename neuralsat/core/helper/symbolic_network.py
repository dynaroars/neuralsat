...
class SymbolicNetwork:
    ...
    def symbolic_input(self):
        ...
        return torch.concat([default_input, torch.zeros(*self.net.input_shape[1:], 1)], dim=-1).to(self.dtype).to(self.device)
...
class SymbolicLinear:
    ...
    def __init__(self, layer, device):
        ...
        self.weight = layer.weight.to(self.dtype).to(device)
        self.bias = layer.bias.to(self.dtype).to(device)
...
class SymbolicConv2d:
    ...
    def __init__(self, layer, device):
        self.dtype = arguments.Config['dtype']
        self.weight = layer.weight.to(self.dtype).to(device) # OUT x IN x K1 x K2
        self.bias = layer.bias.to(self.dtype).to(device)
