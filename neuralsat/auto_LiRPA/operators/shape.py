...
class BoundSqueeze(Bound):
    ...
    def bound_forward(self, dim_in, x):
        ...
        if len(self.input_shape) == 0:
            ...
            lw, lb = x.lw, x.lb
            uw, ub = x.uw, x.ub
            # lw, lb = x.lw.squeeze(self.axes), x.lb.squeeze(self.axes-1)
            # uw, ub = x.uw.squeeze(self.axes), x.ub.squeeze(self.axes-1)
        return LinearBound(lw, lb, uw, ub)
...
class BoundGather(Bound):
    ...
    def forward(self, x, indices):
        ...
        if indices.ndim == 0 or (indices.ndim == 1 and indices.sum() == 0):
            return torch.index_select(x, dim=self.axis, index=indices).squeeze(self.axis)
        elif self.axis == 0:
            # print(x.shape, self.axis, indices, torch.index_select(x, dim=self.axis, index=indices.reshape(-1)).shape)
            # exit()
            return torch.index_select(x, dim=self.axis, index=indices.reshape(-1)).reshape(*indices.shape, x.shape[-1])
    ...
    def bound_forward(self, dim_in, x, indices):
        # print(self, self.indices, self.indices.shape)
...
class BoundTranspose(Bound):
    ...
    def bound_backward(self, last_lA, last_uA, x):
        def _bound_oneside(last_A):
            ...
            #     print(last_A.shape, self.input_shape)
            #     print(last_A.unstable_idx)
            #     print(self.perm_inv_inc_one)
            #     # (batch_size, out_c * out_h * out_w, input_c, input_h, input_w) or (batch_size, unstable_size, input_c, input_h, input_w)
                
            #     # Non-sparse pieces, with shape (out_c, batch, out_h, out_w, c, h, w).
            #     # output_channel, batch_size, output_x, output_y = pieces.shape[:4]
                
            #     patches = last_A.patches.permute(3, 1, 2, 0, 4, 5, 6)
            #     print(patches.shape)
            #     return patches
            #     next_A = patches_to_matrix(
            #         patches, 
