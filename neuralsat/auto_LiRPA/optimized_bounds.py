...
def _set_beta(
        ...
        lr_coeffs, opt_bias, lr_beta, dense_coeffs_mask):
    """
    ...
    return betas, best_betas, coeffs, dense_coeffs_mask, best_coeffs, biases, best_biases
...
def get_optimized_bounds(
    ...
    if init_alpha:
        ...
        self.init_slope(x, 
                        share_slopes=opts['use_shared_alpha'],
                        method=method, 
                        c=C, 
    ...
    if alpha:
        best_alphas = _set_alpha(optimizable_activations=optimizable_activations, 
                                 parameters=parameters, 
                                 alphas=alphas, 
                                 lr=lr_alpha)

    if beta:
        ret_set_beta = _set_beta(self, 
                                 relus=relus, 
    ...
    for i in range(iteration):
        ...
        if 0:
            ...
            # total_fake_loss = 0.0
            # for node in optimizable_activations:
                # mask_1 = ((node.inputs[0].lower * node.inputs[0].upper) < 0) * (node.inputs[0].upper <= thresh_) 
                # mask_2 = ((node.inputs[0].lower * node.inputs[0].upper) < 0) * (node.inputs[0].lower >= -thresh_) 
            interm_uppers = [(((_.inputs[0].lower * _.inputs[0].upper) < 0) * ( _.inputs[0].upper < thresh_) * _.inputs[0].upper).flatten(1).sum(dim=1) for _ in optimizable_activations if _.inputs[0].lower.requires_grad]
            interm_lowers = [(((_.inputs[0].lower * _.inputs[0].upper) < 0) * ( _.inputs[0].lower > -thresh_)* _.inputs[0].lower).flatten(1).sum(dim=1) for _ in optimizable_activations if _.inputs[0].upper.requires_grad]
            fake_loss_1 = torch.stack(interm_uppers).sum(dim=0) if len(interm_uppers) else None
            fake_loss_2 = torch.stack(interm_lowers).sum(dim=0) if len(interm_lowers) else None
            # n_unstable = torch.stack([(((_.inputs[0].lower * _.inputs[0].upper) < 0) * ((_.inputs[0].lower * _.inputs[0].upper))).mean(dim=1) for _ in optimizable_activations]).mean(dim=0)
            # print(f'\t- loss', loss_)
            # print(f'\t- fake_loss_1', fake_loss_1)
            # print(f'\t- fake_loss_2', fake_loss_2)
            # print(f'\t- bounds', optimizable_activations[0].inputs[0].lower.requires_grad)
            # for n_ in optimizable_activations:
            #     print('\t\t- node:', n_, n_.inputs[0].lower.requires_grad)
            
            
            # print(f'\t- n_unstable', n_unstable)
            if (fake_loss_1 is not None) and (fake_loss_2 is not None):
                fake_loss = (fake_loss_1 - fake_loss_2) # - n_unstable
                # fake_loss = -n_unstable
                fake_stop_criterion = (not stop_criterion) if isinstance(stop_criterion, bool) else stop_criterion.logical_not()
                total_loss = (fake_loss * fake_stop_criterion).unsqueeze(-1)
                # total_loss = fake_loss.sum()
                # total_loss += total_fake_loss.unsqueeze(-1)
                # total_loss = fake_loss
                # print('stable loss:', i, fake_loss)
        ...
        with torch.no_grad():
            ...
            if keep_best:
                if best_ret_u is not None:
                    ret_upd = _update_best_ret(full_ret_bound=full_ret_u, 
                                               best_ret_bound=best_ret_u, 
                                               full_ret=full_ret, 
                                               best_ret=best_ret, 
                ...
                if best_ret_l is not None:
                    ret_upd = _update_best_ret(full_ret_bound=full_ret_l, 
                                               best_ret_bound=best_ret_l, 
                                               full_ret=full_ret, 
                                               best_ret=best_ret, 
                                               need_update=need_update, 
                                               idx=0)
    ...
    if (intermediate_layer_bounds is not None and not fix_intermediate_layer_bounds):
        for l in self._modules.values():
            if (l.name in intermediate_layer_bounds.keys() and hasattr(l, 'lower')):
                l.lower = torch.max(
    ...
    # if (opts['pruning_in_iteration'] and decision_thresh is not None and full_l.numel() > 0):
    #     stime = time.time()
    #     with torch.no_grad():
    #         if isinstance(decision_thresh, torch.Tensor):
    #             if decision_thresh.shape[-1] == 1:
...
def init_slope(
        ...
        c=None, bound_lower=True, bound_upper=False, final_node_name=None,
...
def get_refined_intermediate_bounds(self):
    intermediate_bounds = {
        node.inputs[0].name: [
                node.inputs[0].lower.to(self.device) if hasattr(node.inputs[0], 'lower') else None, 
                node.inputs[0].upper.to(self.device) if hasattr(node.inputs[0], 'upper') else None
            ] for node in self.perturbed_optimizable_activations
    }
    if any([(None in _ ) for _ in intermediate_bounds.values()]):
        return None
