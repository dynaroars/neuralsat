...
from src.algorithm.verification_objectives import ArbitraryObjective, LocalRobustnessObjective
...
def run_benchmark(
    ...
    if targets is None:
        targets = model(
            torch.Tensor(nnet.normalize_input(images.reshape(batch_size, -1)).reshape(
                (batch_size, *img_shape)))).argmax(dim=1).numpy()
    ...
    with open(result_path, 'w', buffering=1) as f:
        ...
        f.write(f"Benchmarking with:" f"Timeout {timeout} seconds \n" + f"Model path: {model_path} \n\n")
            ...
            for i in tqdm(range(len(images))):
                ...
                if False:
                    ...
                    for j in range(10):
                        ...
                        new_objective = [0] * 11
                        new_objective[j] = -1
                        new_objective[targets[i]] = 1
                        objectives.append([new_objective])
                    objective = ArbitraryObjective(np.array(objectives), input_bounds, output_size=10)
