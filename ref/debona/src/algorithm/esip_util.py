...
def concretise_symbolic_bounds_jit(input_bounds: np.array, symbolic_bounds: np.array, outward_round: float = 0):
    ...
    for j in prange(layer_size):
        ...
        for i in range(input_bounds[0].shape[0]):

            coeff = symbolic_bounds[j, i]

            if coeff < 0:
                temp_lower += coeff * input_bounds[1][i, 1] - outward_round
                temp_upper += coeff * input_bounds[0][i, 0] + outward_round
            else:
                temp_lower += coeff * input_bounds[0][i, 0] - outward_round
                temp_upper += coeff * input_bounds[1][i, 1] + outward_round
