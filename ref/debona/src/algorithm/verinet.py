...
class VeriNet:
    ...
    def __init__(self,
        ...
        self._counter_example: Optional[Any] = None
        ...
        self.logger = get_logger(LOGS_LEVEL, __name__,
                                 "../../logs/", "verinet_log")
    ...
    def verify(self,
        ...
        self._counter_example = mp.Array("f", np.zeros(
            verification_objective.input_size, dtype=np.float32))
    ...
    def _one_shot_approximation(self):
        ...
        solver = VeriNetWorker(self._model_nn,
                               verification_objective=deepcopy(
                                   self._verification_objective),
                               no_split=True,
    ...
    def _put_poison_pills(self):
        ...
        for i in range(self._max_procs):
            self.logger.debug(
                f"Added poison pill {i + 1} of {self._max_procs}")
            # "Poison pill for killing child processes"
            self._branch_queue.put(None)
    ...
    def _start_worker(self):
        ...
        while True:
            ...
            # print("Current queue: ", (self._branch_queue.__dict__))
            branch = self._branch_queue.get()
            self.logger.debug(
                f"Worker {self} retrieved branch {branch} from queue")
            with self._work_lock:
                if branch is None:
                    self._active_procs.value -= 1
                    if self._active_procs.value == 0:
                        self._all_children_done.set()
                    self.logger.debug(f"Worker {self} exited")
                    return
            ...
            self._finished_subtree(
                solver.max_depth, solver.branches_explored, solver.status, solver.counter_example)

    def _finished_subtree(self,
                          max_depth: int,
