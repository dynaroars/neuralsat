...
class VeriNetWorker:
    def __init__(self,
        ...
        self._counter_example: Optional[torch.Tensor] = None
    ...
    @property
    def counter_example(self) -> Optional[torch.Tensor]:
    ...
    def verify(self, start_branch: Branch, finished_flag: Optional[synchronize.Event],
               needs_branches: Optional[Callable], put_queue: Optional[Callable],
               queue_depth: int) -> Optional[Status]:
        """
        ...
        while True:
            ...
            if (needs_branches is not None and put_queue is not None and len(self._branches) >= queue_depth
                    and (self._branches[-1].depth - self._branches[0].depth) >= queue_depth and needs_branches()):
            ...
            if self._verbose:
                print(
                    f" Depth: {current_branch.depth}, _status: {self._status}")
    ...
    def _grad_descent_counter_example(self,
        ...
        x = torch.Tensor(lp_counter_example).clone().reshape(  # type: ignore
            1, *self._verification_objective.input_shape)
        ...
        optimizer = optim.Adam(
            [x], lr=self._gradient_descent_step, betas=(0.5, 0.9))

        old_loss = 1e10
        input_bounds = torch.Tensor(
            self._verification_objective.input_bounds_flat)  # type: ignore
        ...
        for _ in range(self._gradient_descent_max_iters):
            ...
            x = x.view(-1)
            x.data[x < input_bounds[:, 0]] = input_bounds[:,
                                                          0][x < input_bounds[:, 0]]
            x.data[x > input_bounds[:, 1]] = input_bounds[:,
                                                          1][x > input_bounds[:, 1]]
    ...
    def _get_logits(self, x: torch.Tensor) -> torch.Tensor:
        ...
        try:
            ...
            raise LogitsException(
                "Neural network has to implement attribute logits") from e
    ...
    def _branch(self, current_branch: Branch) -> bool:
        ...
        lower = self._bounds._get_concrete_bounds(layer - 1)[0][node][0]
        upper = self._bounds._get_concrete_bounds(layer - 1)[1][node][1]
        ...
        split_forced[layer - 1][node,
                                1] = split_x if split_x < old_forced else old_forced

        new_split = {"layer": layer, "node": node,
                     "split_x": split_x, "upper": False}
        ...
        split_forced[layer - 1][node,
                                0] = split_x if split_x > old_forced else old_forced

        new_split = {"layer": layer, "node": node,
                     "split_x": split_x, "upper": True}
        split_list = current_branch.split_list.copy()
        split_list.append(new_split)
        new_branch = Branch(current_branch.depth + 1, split_forced, split_list)
        new_branch.safe_classes = current_branch.safe_classes.copy()
    ...
    def _switch_branch(self, current_branch: Branch, new_branch: Branch):
        ...
        # Backtracking, recalculate from first differing layer, but do not recalculate input bounds to first layer
            ...
            min_layer = min(
                [split["layer"] for split in current_branch.split_list[new_branch.depth - 1:]])
