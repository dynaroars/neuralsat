...
class Branch:
    ...
    def __init__(self, depth: int, forced_input_bounds: np.array, split_list: list):
        ...
        self._lp_solver_constraints: Optional[list] = None
    ...
    def add_constr_to_solver(bounds: ESIP, lp_solver: LPSolver, split: np.array) -> grb.Constr:
        ...
        symb_input_bounds = bounds._get_symbolic_equation(layer-1)

        if upper:
            constr = lp_solver.grb_solver.addConstr(
                grb.LinExpr(symb_input_bounds[1][node][:-1], input_vars) + symb_input_bounds[1][node][-1] >= split_x)
        else:
            constr = lp_solver.grb_solver.addConstr(
                grb.LinExpr(symb_input_bounds[0][node][:-1], input_vars) + symb_input_bounds[0][node][-1] <= split_x)
    ...
    def update_constrs(self, bounds: ESIP, solver: LPSolver, old_split_list: list, old_constr_list: list):
        ...
        re_add_idx = [i for i in range(
            self.depth - 1) if old_split_list[i]["layer"] > min_layer]
        ...
        for i in range(self.depth - 1):
            ...
            assert self._lp_solver_constraints is not None
