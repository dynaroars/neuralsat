...
class LPSolver:
    ...
    def _init_variables(self):
        ...
        self._input_variables = (self._grb_solver.addVars(range(self._input_size),
                                                          lb=-grb.GRB.INFINITY,
                                                          ub=grb.GRB.INFINITY,
                                                          vtype=grb.GRB.CONTINUOUS,
                                                          name="Input"))
        self._output_variables = (self._grb_solver.addVars(range(self._output_size),
                                                           lb=-grb.GRB.INFINITY,
                                                           ub=grb.GRB.INFINITY,
                                                           vtype=grb.GRB.CONTINUOUS,
                                                           name="Output"))
    ...
    def set_variable_bounds(self, bounds: ESIP, output_bounds: np.array = None, set_input: bool = True):
        ...
        if set_input:
            input_bounds_lower = bounds.input_constraints[:, 0]
            input_bounds_upper = bounds.input_constraints[:, 1]
        ...
        output_bounds_lower = bounds._get_concrete_bounds(-1)[0][:, 0].copy()
        output_bounds_upper = bounds._get_concrete_bounds(-1)[1][:, 1].copy()
        ...
        if output_bounds is not None:
            ...
            output_bounds_lower[better_lower_idx] = output_bounds[0][better_lower_idx, 0]
            output_bounds_upper[better_upper_idx] = output_bounds[1][better_upper_idx, 1]
