...
class AbstractSShaped(AbstractMapping):
    ...
    def propagate(self, x: np.array, add_bias: bool = True) -> np.array:
        """
        Propagates through the mapping (by applying the activation function or layer-operation).
        ...
        raise NotImplementedError(
            f"propagate(...) not implemented in {self.__name__}")
    ...
    def split_point(self, xl: float, xu: float) -> float:
        ...
        raise NotImplementedError(
            f"split_point(...) not implemented in {self.__name__}")
    ...
    def linear_relaxation(self, bounds_concrete_in: np.array, upper: bool, do_test: bool) -> np.array:
        ...
        Args:
            bounds_concrete_in    : The concrete upper/lower bounds of the input to the nodes
        ...
        lower_bounds_concrete_in = bounds_concrete_in[0][:, 0]
        upper_bounds_concrete_in = bounds_concrete_in[1][:, 1]
        ...
        unequal_bounds_idx = np.argwhere(
            (lower_bounds_concrete_in != upper_bounds_concrete_in)).squeeze()
    ...
    def _tangent_line(self,
                      ...
                      upper: bool,
                      tangent_point: np.array = None) -> np.array:
        """
    ...
    def _intercept_line(self, lower_bounds_concrete_in: np.array, upper_bounds_concrete_in: np.array,
                        upper: bool) -> np.array:
        """
    ...
    def derivative(self, x: np.array) -> np.array:
        ...
        raise NotImplementedError(
            "derivative(...) not implemented in subclass")
    ...
    def _update_xi(self, xi: np.array, x_bound: np.array, upper: bool):
        ...
        raise NotImplementedError(
            "_update_xi(...) not implemented in subclass")
...
class Sigmoid(AbstractSShaped):
    ...
    def propagate(self, x: np.array, add_bias: bool = True) -> np.array:
        """
        Propagates through the mapping by applying the Sigmoid element-wise.
    ...
    def _update_xi(self, xi: np.array, x_bound: np.array, upper: bool):
        ...
        root = (np.sqrt(1 - 4 * (self.propagate(xi) -
                                 self.propagate(x_bound)) / (xi - x_bound))) / 2.
        if upper:
            sxi = 0.5 + root
        else:
            sxi = 0.5 - root
...
class Tanh(AbstractSShaped):
    ...
    def propagate(self, x: np.array, add_bias: bool = True) -> np.array:
        """
        Propagates through the mapping by applying the Tanh element-wise
    ...
    def _update_xi(self, xi: np.array, x_bound: np.array, upper: bool):
        ...
        root = np.sqrt(1 - (self.propagate(xi) -
                            self.propagate(x_bound)) / (xi - x_bound))
        if upper:
            sxi = root
        else:
            sxi = -root
...
class SigmoidNaive(AbstractMapping):
    ...
    def linear_relaxation(self, lower_bounds_concrete_in: np.array, upper_bounds_concrete_in: np.array,
        ...
        a = np.min(np.hstack((self.derivative(lower_bounds_concrete_in).reshape(
            (-1, 1)), self.derivative(upper_bounds_concrete_in).reshape((-1, 1)))),
            axis=1)

        if upper:
            b = self.propagate(upper_bounds_concrete_in) - \
                a * upper_bounds_concrete_in
        else:
            b = self.propagate(lower_bounds_concrete_in) - \
                a * lower_bounds_concrete_in
