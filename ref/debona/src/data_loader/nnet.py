...
class NNET:
    ...
    def __init__(self, path: str = None):
        ...
        self.activation_map_torch = {-1: None, 0: nn.ReLU(), 1: nn.Sigmoid(), 2: nn.Tanh()}
    ...
    def _read_nnet_conv2d(self, file, conv_params: dict):
        ...
        for channel in range(conv_params["out_channels"]):
            ...
            weights[channel] = np.array([np.float64(num) for num in line]).reshape(
                (conv_params["in_channels"], conv_params["kernel_size"], conv_params["kernel_size"]))
    ...
    def from_nnet_to_verinet_nn(self) -> VeriNetNN:
        ...
        for layer_num in range(self.num_layers):
            ...
            if layer_type_num == 0:
                ...
                # self.weights[layer_num] = np.random.choice([-1, 0, 0, 0, 1], self.weights[layer_num].shape)
                # self.biases[layer_num] = self.biases[layer_num] * 0
                # print("Layer", layer_num, "weight", self.weights[layer_num])
                # print("Layer", layer_num, "biases", self.biases[layer_num])
