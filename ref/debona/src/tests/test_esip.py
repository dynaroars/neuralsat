...
class TestNNBounds(unittest.TestCase):
    ...
    def test_prop_equation_through_relaxation(self):
        """
        Tests the output from the _prop_equation_through_relaxation() method against ground truth.
        """

        symb_bounds = np.array([[-3, -2, -1]])
        relax = np.array(([[[0.5, 1]], [[0.5, 2]]]))
        gt_new_symb_bounds = np.array([[-1.5, -1, 0.5]])
        new_symb_bounds = self.bounds_relu._prop_equation_through_relaxation(symb_bounds, relax)
    ...
    def test_prop_error_matrix_through_relaxation(self):
        """
        Tests the output from the _prop_error_matrix_through_relaxation() method against ground truth.
        """
        ...
        new_error_matrix, error_matrix_to_node_indices = \
            self.bounds_relu._prop_error_matrix_through_relaxation(error_matrix, relax, bounds_concrete,
