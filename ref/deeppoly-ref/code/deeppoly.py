...
class DeepPolyFlattenTransformer(nn.Module):
    ...
    def forward(self, bounds):
        b = torch.stack([bounds[0,:,:,:].flatten(), bounds[1,:,:,:].flatten()], 1)
        return b
...
class TestConvNet(nn.Module):
    ...
    def __init__(self):
        ...
        self.conv1 = nn.Conv2d(2, 2, kernel_size=2, stride=1)
        w = torch.stack([torch.ones(1, 8).view(2, 2, 2), -1*torch.ones(1, 8).view(2, 2, 2)]).float()
        self.conv1.weight = nn.Parameter(w)
        self.conv1.bias = torch.nn.Parameter(torch.Tensor([1, 1]))

        self.flatten = nn.Flatten()
        self.linear1 = nn.Linear(8, 3)
        self.linear1.weight = torch.nn.Parameter(torch.arange(1, 25).view(3, 8).float())
        self.linear1.bias = torch.nn.Parameter(torch.Tensor([-1, -2, -3]))

        self.linear2 = nn.Linear(3, 2)
        self.linear2.weight = torch.nn.Parameter(torch.arange(1, 7).view(2, 3).float())
        self.linear2.bias = torch.nn.Parameter(torch.Tensor([1, 2]))

        self.layers = nn.Sequential(*[Normalization(torch.device('cpu')), self.conv1, nn.ReLU(), self.flatten, self.linear1, nn.ReLU(), self.linear2])
...
if __name__ == "__main__":
    ...
    net = TestConvNet()
    x = torch.ones([1, 2, 3, 3]) / 2

    d = DeepPoly(net, 0.1, x, 0, back_sub_steps=10)
