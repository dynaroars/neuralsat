...
N_PROC = __import__('os').cpu_count() // 2
DEBUG = True
...
def build_solver_module(self: 'BoundedModule', x=None, C=None, interm_bounds=None,
                        ...
                        set_input=True, timeout=None, timeout_per_neuron=None, refine=False):
    ...
    if model_type == 'lp':
        ...
        if refine:
            assert all([isinstance(_, BoundRelu) for _ in self.perturbed_optimizable_activations]), \
                'Error: Support ReLU only'
        self._build_solver_refined(x=x, node=final, C=C, model_type=model_type,
                                   solver_pkg=solver_pkg, refine=refine,
...
def _build_solver_refined(self, x, node, C=None, model_type="mip", solver_pkg="gurobi",
                          refine=False, timeout_per_neuron=None):
    if not hasattr(node, 'solver_vars'):
        for n in node.inputs:
            self._build_solver_refined(x=x, node=n, C=C, model_type=model_type,
                                       solver_pkg=solver_pkg, refine=refine,
                                       timeout_per_neuron=timeout_per_neuron)
            ...
            if isinstance(refine_node, (BoundConv, BoundConvTranspose)):
                ...
                if len(candidates):
                    ...
                    if (N_PROC > 1) and (len(candidates) > 1):
                        ...
                        solver_result = [mip_solver_worker(can) for can in candidates]
                    MULTIPROCESS_MODEL = None
                ...
                self.set_bound_opts({'optimize_bound_args': {
                    'enable_beta_crown': True,
                    'stop_criterion_func': lambda x: False,
                }})
                lb_, _ = self.compute_bounds(x=x, C=C, method="crown-optimized",
...
def mip_solver_worker(candidate):
    """Multiprocess worker for solving MIP models in build_solver_refined."""
    ...
    if abs(out_lb) < abs(out_ub):
        vlb, refined, _ = solve_lb(model, v, out_lb, eps=eps)
        neuron_refined = neuron_refined or refined
        if vlb <= 0 or EAGER_OPTIMIZE:
            vub, refined, _ = solve_ub(model, v, out_ub, eps=eps)
            neuron_refined = neuron_refined or refined
        else:
            vub = out_ub
    else:
