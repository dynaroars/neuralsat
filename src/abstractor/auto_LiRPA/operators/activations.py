    ...
    def bound_backward(self, last_lA, last_uA, x, **kwargs):
        ...
        if last_uA is not None:
            ...
            if last_uA.min() >= 0:
                ...
                ubias = self.get_bias(last_uA_pos, upper_b) + self.get_bias(last_uA_neg, lower_b)
        else:
            uA, ubias = None, 0
        if last_lA is not None:
            if last_lA.max() <= 0:
                ...
                lbias = self.get_bias(last_lA_pos, lower_b) + self.get_bias(last_lA_neg, upper_b)
        else:
            lA, lbias = None, 0
...
class BoundATenHeaviside(BoundOptimizableActivation):
    ...
    def bound_backward(self, last_lA, last_uA, *x, start_node=None, start_shape=None, **kwargs):
        x = x[0]
        if x is not None:
            lb_r = x.lower
            ub_r = x.upper
...
class BoundSqr(BoundOptimizableActivation):
    ...
    def bound_relax(self, x, init=False, dim_opt=None):
        ...
        self.add_linear_relaxation(mask=None, type='upper', k=upper_k, x0=x.lower)
...
class BoundHardTanh(BoundActivation):
    ...
    def bound_backward(self, last_lA, last_uA, x, min_val, max_val, start_node=None, unstable_idx=None, reduce_bias=True, **kwargs):
        if self.is_input_perturbed(1) or self.is_input_perturbed(2):  # Checking if min_value and max_value are not perturbed
            raise NotImplementedError(f'{self.activation_name} is unsupported with perturbed min_val and max_val')
...
class BoundMultiPiecewiseNonlinear(BoundOptimizableActivation):
    ...
    def bound_backward(self, last_lA, last_uA, x, weight, offset, reduce_bias=True, start_node=None, **kwargs):
        assert not self.is_input_perturbed(1)
        ...
        lA, lb = _bound_oneside(last_lA, relu_lower_k_for_lA, relu_lower_b, relu_upper_k,        relu_upper_b, weight, offset, reduce_bias)
        uA, ub = _bound_oneside(last_uA, relu_upper_k,        relu_upper_b, relu_lower_k_for_uA, relu_lower_b, weight, offset, reduce_bias)

        return [(lA, uA), (None, None), (None, None)], lb, ub

    def _init_opt_parameters_impl(self, size_spec, **kwargs):
        alpha = torch.empty(2, size_spec, *self.init_lower_k.shape, device=self.init_lower_k.device)
