...
class BoundTranspose(Bound):
    ...
    def build_solver(self, *v, model, C=None, model_type="mip", solver_pkg="gurobi"):
        if isinstance(v[0], Tensor):
            ...
            # v[0] has no batch dim; perm may include batch dim (dim 0 → stays at 0)
            arr = np.array(v[0], dtype=object)
            arr = arr[np.newaxis]                    # add batch dim
            arr = np.transpose(arr, self.perm)       # apply full perm
            self.solver_vars = arr[0].tolist()       # remove batch dim
