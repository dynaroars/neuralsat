...
class MulHelper:
    ...
    @staticmethod
    def interpolated_relaxation(x_l: Tensor, x_u: Tensor, y_l: Tensor, y_u: Tensor, r_l: Optional[Tensor] = None, r_u: Optional[Tensor] = None, middle: bool = False):
    ...
    @staticmethod
    def get_relaxation(x_l: Tensor, x_u: Tensor, y_l: Tensor, y_u: Tensor, opt_stage: Optional[str], alphas: Optional[Dict[str, Tensor]], start_name: Optional[str], middle: bool = False):
        if opt_stage in ['opt', 'reuse']:
            assert x_l.ndim == y_l.ndim
            ns = start_name
            alphas[ns].data[:] = alphas[ns].data[:].clamp(min=0, max=1)
            return MulHelper.interpolated_relaxation(x_l, x_u, y_l, y_u, alphas[ns][:2], alphas[ns][2:4])
        else:
            return MulHelper.interpolated_relaxation(x_l, x_u, y_l, y_u, middle=middle)
...
class BoundMul(BoundOptimizableActivation):
    ...
    def get_relaxation_opt(self, x_l, x_u, y_l, y_u):
        return self.mul_helper.get_relaxation(x_l, x_u, y_l, y_u, self.opt_stage, getattr(self, 'alpha', None), getattr(self, '_start', None), middle=self.middle)
    ...
    def bound_relax(self, x, y, init=False, dim_opt=None):
        if init:
            pass
        alpha_l, beta_l, gamma_l, alpha_u, beta_u, gamma_u = self.get_relaxation_opt(x.lower, x.upper, y.lower, y.upper)
    ...
    def bound_backward_constant(self, last_lA, last_uA, x, y, op=None, reduce_bias=True, **kwargs):
        assert reduce_bias
        ...
        lAx = None if (not x.perturbed or last_lA is None) else self.broadcast_backward(op(last_lA, factor), x)
        uAx = None if (not x.perturbed or last_uA is None) else self.broadcast_backward(op(last_uA, factor), x)
        lAy = None if (not y.perturbed or last_lA is None) else self.broadcast_backward(op(last_lA, factor), y)
        uAy = None if (not y.perturbed or last_uA is None) else self.broadcast_backward(op(last_uA, factor), y)
    ...
    def bound_backward(self, last_lA, last_uA, x, y, start_node=None, **kwargs):
        ...
        if self.is_linear_op:
            ...
            ret = self.bound_backward_both_perturbed( last_lA, last_uA, x, y, **kwargs)
        return ret

    def bound_backward_both_perturbed(self, last_lA, last_uA, x, y, reduce_bias=True, **kwargs):
        self.bound_relax(x, y)

        def _bound_oneside(last_A, alpha_pos, beta_pos, gamma_pos, alpha_neg, beta_neg, gamma_neg, opt=False):
            if last_A is None:
                return None, None, 0
            ...
            if type(last_A) == Patches:
                ...
                A_x, bias = multiply_by_A_signs(last_A, alpha_pos, alpha_neg, gamma_pos, gamma_neg)
                ...
                if reduce_bias:
                    if opt:
                        bias = torch.einsum('sb...,sb...->sb', last_A_pos, gamma_pos) + torch.einsum('sb...,sb...->sb', last_A_neg, gamma_neg)
                    else:
                        bias = self.get_bias(last_A_pos, gamma_pos.squeeze(0)) + self.get_bias(last_A_neg, gamma_neg.squeeze(0))
                else:
                    assert not opt
        ...
        if self.opt_stage in ['opt', 'reuse']:
            lA_x, lA_y, lbias = _bound_oneside(last_lA, alpha_l[0], beta_l[0], gamma_l[0], alpha_u[0], beta_u[0], gamma_u[0], opt=True)
            uA_x, uA_y, ubias = _bound_oneside(last_uA, alpha_u[1], beta_u[1], gamma_u[1], alpha_l[1], beta_l[1], gamma_l[1], opt=True)
        else:
    ...
    def bound_forward_both_perturbed(self, dim_in, x, y):
        ...
        alpha_l, beta_l, gamma_l, alpha_u, beta_u, gamma_u = MulHelper.get_forward_relaxation(x.lower, x.upper, y.lower, y.upper, self.opt_stage, getattr(self, 'alpha', None), self._start)
        ...
        lb = alpha_l.clamp(min=0) * x_lb + alpha_l.clamp(max=0) * x_ub + beta_l.clamp(min=0) * y_lb + beta_l.clamp(max=0) * y_ub + gamma_l
        ...
        ub = alpha_u.clamp(max=0) * x_lb + alpha_u.clamp(min=0) * x_ub + beta_u.clamp(max=0) * y_lb + beta_u.clamp(min=0) * y_ub + gamma_u
